...
exports.list = async (_req, res) => {
  try{
    const items = await TechStack.find({}).sort({ name: 1 });
    res.json({ items });
...
exports.create = async (req, res) => {
  try{
    ...
    const item = await TechStack.create({ name, active: !!active });
...
exports.update = async (req, res) => {
  try{
    ...
    const item = await TechStack.findByIdAndUpdate(req.params.id, payload, { new: true });
    if(!item) return res.status(404).json({ error: 'Not found' });
    res.json({ item });
  }catch(err){ res.status(500).json({ error: 'Server error', details: err.message }); }
};
